...
function buildReengagementHtml(firstName: string, venueName: string, offerExpiry: string, unsubToken: string): string {
  return `<!DOCTYPE html>
<html lang="en">
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: eois, error } = await supabase
    ...
    .select('id, venue_name, contact_name, email, created_at, unsubscribe_token')
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          html: buildReengagementHtml(firstName, eoi.venue_name, offerExpiry, eoi.unsubscribe_token || ''),
